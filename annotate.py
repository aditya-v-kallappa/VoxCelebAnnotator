...
class Annotate:
    ...
    def __init__(self, args):
        ...
        if not self.args.disable_alignment:
            self.diarize_model = whisperx.DiarizationPipeline(use_auth_token='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', device=args.device)
    ...
    def get_transcript(self,  audio_file):
        ...
        if self.args.fast_transcript:
            ...
            segments, transcript_info = self.asr_model.transcribe(audio_file)
    ...
    def align(self, audio_file, result):  
        ...
        model_a, metadata = whisperx.load_align_model(language_code=result['language'], device=self.args.device)
        audio = whisperx.load_audio(audio_file)
        result = whisperx.align(result['segments'], model_a, metadata, audio, self.args.device, return_char_alignments=False)
    ...
    def annotate_audio(self, audio_file):
        ...
        if not self.args.disable_asr:
            result, transcript_info, transcript_result_info = self.get_transcript(audio_file)
            annotations['transcript'] = ' '.join([r['text'] for r in result['segments']])
            annotations['transcript_info'] = transcript_info
            annotations['transcript_result_info'] = transcript_result_info
        
            if not self.args.disable_alignment:
                result = self.align(audio_file, result)
